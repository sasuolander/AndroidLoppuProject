...
class NewstoriesFragment:Fragment(){
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        v = LayoutInflater.from(context).inflate(R.layout.new_fragment_item_list,
                container,false)
        recyclerView= v.findViewById(R.id.list)
        recyclerView.layoutManager=LinearLayoutManager(context)

        if(MyApplication.instance?.newstoriesListInstant!!.isNotEmpty()){
            val listForView=MyApplication.instance?.newstoriesListInstant
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
                if(MyApplication.instance?.newstoriesListInstant!!.isEmpty()){
