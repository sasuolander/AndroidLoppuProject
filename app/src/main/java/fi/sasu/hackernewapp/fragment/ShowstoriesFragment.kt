...
class ShowstoriesFragment:Fragment(){
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        v = LayoutInflater.from(context).inflate(R.layout.show_story_fragment_item_list,
                container,false)
        recyclerView= v.findViewById(R.id.list)
        recyclerView.layoutManager=LinearLayoutManager(context)

        if(MyApplication.instance?.showstoriesListInstant!!.isNotEmpty()){
            val listForView=MyApplication.instance?.showstoriesListInstant
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
                if(MyApplication.instance?.showstoriesListInstant!!.isEmpty()){
                    val testi = Model(1,"testi",
                            0,"testi",0)
