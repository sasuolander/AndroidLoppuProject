...
class JobstoriesFragment:Fragment(){
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        v = LayoutInflater.from(context).inflate(R.layout.job_fragment_item_list,
                container,false)
        recyclerView= v.findViewById(R.id.list)
        recyclerView.layoutManager=LinearLayoutManager(context)

        if(MyApplication.instance?.jobstoriesListInstant!!.isNotEmpty()){
            val listForView=MyApplication.instance?.jobstoriesListInstant
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
                if(MyApplication.instance?.jobstoriesListInstant!!.isEmpty()){
